...
public class Aggregator {
    ...
    public List<String> sendTasksToWorkers(List<String> server_list, List<String> tasks) {
        ...
        for (int i = 0; i < futures.length; i++) {
            ...
            + (i+ 1) + "]:" + task);
            futures[i] = webClient.sendTask(server_list.get(i)+task);
        }
        ...
        while (bandera) {
            ...
            if(futures[index_server].isDone()) {
                ...
                if (index_next_task< tasks.size()) {
                    ...
                    futures[index_server] = webClient.sendTask(server_list.get(index_server)+task);
                    System.out.println("Termino future[" + index_server + "] (" + server_list.get(index_server)+task + "). Ahora realizara tarea[" + (index_next_task + 1) + "]:\"" + task+"\"");
